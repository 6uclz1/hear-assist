type Props = { level: number; rms: number | null; db: number | null; error: string };

export function InputMeter({ level, rms, db, error }: Props) {
  ...
  return (
    <section className="meter-card" aria-labelledby="input-level-title">
      <div className="meter-heading">
        ...
        <output>{db === null || rms === null ? '—' : `RMS ${rms.toFixed(3)} · ${db.toFixed(0)} dBFS`}</output>
      </div>
